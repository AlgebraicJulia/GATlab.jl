...
Context(c::Context,tc::Union{TermCon, AbstractVector{TermCon}}; name="") = 
  Context(1+c.depth, TheoryExtTerm(codom(c),tc;name=name))
...
@struct_hash_equal struct TheoryChunck
...
struct TheoryExt <: Theory
  ...
  parent::Theory
  tc::TheoryChunk
  function TheoryExt(tname,thry,typecons,termcons,axioms)
    for c in [termcons..., typecons..., axioms...]
      e = "Bad context for $(name(c))  in \"$tname\" extending $(name(thry))"
      dom(ctx(c)) == thry || error(e)
    end
...
TheoryExtTerm(p,tc::AbstractVector{TermCon}; name="") =
