...
@theory ThLawlessMonCat <: ThCategory begin
  A ⊗ B :: Ob ⊣ [(A,B)::Ob]
  f₁ ⊗′ f₂ :: Hom(A₁ ⊗ A₂, B₁ ⊗ B₂) ⊣ [(A₁, A₂, B₁, B₂)::Ob, f::Hom(A₁, B₁), g::Hom(A₂, B₂)]
  I :: Ob
end

@theory ThCoherencelessMonCat <: ThCategory begin
  assocr(A,B,C) :: Hom((A ⊗ B) ⊗ C, A ⊗ (B ⊗ C)) ⊣ [(A,B,C)::Ob]
  assocl(A,B,C) :: Hom(A ⊗ (B ⊗ C), (A ⊗ B) ⊗ C) ⊣ [(A,B,C)::Ob]
  assocr(A,B,C) ⋅ assocl(A,B,C) == id((A ⊗ B) ⊗ C) ⊣ [(A,B,C)::Ob]
  assocl(A,B,C) ⋅ assocr(A,B,C) == id(A ⊗ (B ⊗ C)) ⊣ [(A,B,C)::Ob]
  lunitf(A) :: Hom(A ⊗ I, A) ⊣ [A::Ob]
  lunitb(A) :: Hom(A, A ⊗ I) ⊣ [A::Ob]
  lunitf(A) ⋅ lunitb(A) == id(A ⊗ I) ⊣ [A::Ob]
  lunitb(A) ⋅ lunitf(A) == id(A) ⊣ [A::Ob]
